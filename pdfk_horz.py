...
def create_pivot(df, col, dtype="int"):
    ...
    last = p.head(1); latest_values[col] = {"Tarih": last.index[0].strftime("%d.%m.%Y"), "Veriler": last.iloc[0].to_dict()}
    out = p.reset_index(); out["Tarih"] = out["Tarih"].dt.strftime("%d.%m.%Y")
...
def safe(x): 
    return "" if pd.isna(x) or (isinstance(x,float) and np.isinf(x)) else (round(float(x),5) if isinstance(x,float) else int(x) if isinstance(x,int) else str(x))
...
def latest_vertical():
    ...
    return pd.DataFrame(rows,columns=["Hisse_Kodu","Pd_Carpan","Fk_Carpan","Aktifkarlilik","Ozkarlilik","Ozkaynak","Aktifler","Netborc","Yillik_Kar","Sermaye"])
