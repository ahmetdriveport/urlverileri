...
def fiyat_hacim_tek_gun(g,a,y):
    ...
    if not t: return []
    ts=f"{g:02d}.{a:02d}.{y}"
    return [{"Tarih":ts,"Hisse":c[0],"Kapanış":temizle_sayi(c[1]),
             "Yüksek":temizle_sayi(c[4]),"Düşük":temizle_sayi(c[5]),
...
def pivotla(df,kolon,takip_hisseler,tarih_listesi,haftalik=False):
    ...
    df=df.dropna(subset=["Tarih","Hisse",kolon]); df[kolon]=df[kolon].apply(temizle_sayi)
    if haftalik:
        df["Hafta"]=df["Tarih"].dt.to_period("W"); df["Gun"]=df["Tarih"].dt.dayofweek
        tercih=[4,3,2,1,0]; haftalik=[]
        for hafta,grup in df.groupby("Hafta"):
            for gun in tercih:
                secim=grup[grup["Gun"]==gun]
                if not secim.empty: haftalik.append(secim); break
