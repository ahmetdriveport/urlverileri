...
def clean_numeric_series(s):
    s = s.replace('', pd.NA).replace('<NA>', pd.NA)
    s = s.astype(str).str.replace(',', '.', regex=False)
    s = s.replace('nan', pd.NA).replace('<NA>', pd.NA)
...
def calculate_macd(series, fast=12, slow=26, signal=9):
    s = pd.to_numeric(series, errors="coerce").dropna()
    if s.empty:
        return pd.DataFrame(index=series.index, columns=["MACD","SIGNAL","HIST"], dtype=float)
...
def hesapla_indikatorler(df, tanimlar):
    ...
    for tanim in tanimlar:
        kind = tanim["kind"]; params = tanim.get("params", {}); output = tanim["output"]
        try:
            if kind == "ema":
                ...
                if isinstance(output, dict):
                    for kaynak, hedef in output.items():
                        sonuc[hedef] = cikti.get(kaynak).apply(normalize)
            ...
            print(f"❌ {kind} hesaplama hatası: {e}")
...
def yukle_ayarlar(path="data/indicators.yaml"):
    ...
    return data["indikatorler"]

def main():
    print("📁 [indikator.py] İndikatör modülü başlatıldı.")
    ...
    try:
        ...
        # 3️⃣ master tarih listesi kapanıştan
        master_dates = pd.to_datetime(df_close.index, dayfirst=True, errors="coerce")

        # 4️⃣ takip hisseleri dates.csv’den
        df_csv = pd.read_csv("data/dates.csv", encoding="utf-8")
        semboller = df_csv.iloc[:,1].dropna().unique().tolist()

        sayfa_df = {}

        # 5️⃣ her sembol için indikatör hesapla
        for sembol in semboller:
            try:
                ...
                sonuc = hesapla_indikatorler(df_symbol, tanimlar)
                if not sonuc:
                    continue
